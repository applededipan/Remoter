...
#define NUM_STICKS             4  //四路摇杆
#define NUM_POTS               2  //两路微调
...
PACK(typedef struct {
  uint8_t  page;
  uint8_t  state;     //! 1: 编辑模式   0: 浏览模式
  uint8_t  subState;  //! 每个菜单具体运行的状态
})OpentxMenu;
...
PACK(typedef struct {
  ...
  uint16_t  flightMode;     //! set flight mode 
  uint16_t  gimbalMode;     //! set gimbal mode
  uint16_t  vtolMode;       //! set vtol mode :1000 or 2000
  uint16_t  throwMode;      //! set throw mode :1000 or 2000
  uint32_t  key;  
  uint8_t   sdCardError;    //! 1:error   0:ok
  uint8_t   ftpReady;
  ...
  uint8_t   firmwareUpdate; //! 0: no update   1: update   2: update rejected
