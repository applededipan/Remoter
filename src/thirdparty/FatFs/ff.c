...
/                   Changed argument of f_chdrive(), f_mkfs(), diskRead() and diskWrite().
...
)
{
	...
	if (fs->wflag) {	/* Write back the sector if it is dirty */
		...
		if (diskWrite(fs->drv, fs->win, wsect, 1) != RES_OK) {
			res = FR_DISK_ERR;
		} else {
			fs->wflag = 0;
...
)
{
	...
	if (sector != fs->winsect) {	/* Window offset changed? */
		...
		if (res == FR_OK) {			/* Fill sector window with new data */
			if (diskRead(fs->drv, fs->win, sector, 1) != RES_OK) {
...
)
{
	...
	if (res == FR_OK) {
		...
		if (fs->fs_type == FS_FAT32 && fs->fsi_flag == 1) {
			...
			diskWrite(fs->drv, fs->win, fs->winsect, 1);
			fs->fsi_flag = 0;
		}
		/* Make sure that no pending write process in the physical drive */
		if (diskIoctl(fs->drv, CTRL_SYNC, 0) != RES_OK)
			res = FR_DISK_ERR;
	}
	...
	if (clst < 2 || clst >= fs->n_fatent) {	/* Check range */
		...
		while (clst < fs->n_fatent) {			/* Not a last link? */
			...
			if (ecl + 1 == nxt) {	/* Is next cluster contiguous? */
				...
				diskIoctl(fs->drv, CTRL_TRIM, rt);				/* Erase the block */
				scl = ecl = nxt;
			}
#endif
...
)
{
	...
	*rfs = fs;							/* Return pointer to the file system object */
	...
	stat = diskInit(fs->drv);	/* Initialize the physical drive */
	if (stat & STA_NOINIT)				/* Check if the initialization succeeded */
		return FR_NOT_READY;			/* Failed to initialize due to no medium or hard error */
	if (!_FS_READONLY && wmode && (stat & STA_PROTECT))	/* Check disk write protection if needed */
		return FR_WRITE_PROTECTED;
#if _MAX_SS != _MIN_SS						/* Get sector size (multiple sector size cfg only) */
	if (diskIoctl(fs->drv, GET_SECTOR_SIZE, &SS(fs)) != RES_OK
...
)
{
	...
	if (getDiskStatus(fil->fs->drv) & STA_NOINIT)
		return FR_NOT_READY;
	...
	for ( ;  btr;								/* Repeat until all data read */
		...
		if ((fp->fptr % SS(fp->fs)) == 0) {		/* On the sector boundary? */
			...
			if (cc) {							/* Read maximum contiguous sectors directly */
				...
				if (diskRead(fp->fs->drv, rbuff, sect, cc) != RES_OK)
			...
			if (fp->dsect != sect) {			/* Load data sector if not in cache */
				...
				if (fp->flag & FA__DIRTY) {		/* Write-back dirty sector cache */
					if (diskWrite(fp->fs->drv, fp->buf, fp->dsect, 1) != RES_OK)
						ABORT(fp->fs, FR_DISK_ERR);
					fp->flag &= ~FA__DIRTY;
				}
#endif
				if (diskRead(fp->fs->drv, fp->buf, sect, 1) != RES_OK)	/* Fill sector cache */
					ABORT(fp->fs, FR_DISK_ERR);
			}
#endif
			fp->dsect = sect;
	...
	for ( ;  btw;							/* Repeat until all data written */
		...
		if ((fp->fptr % SS(fp->fs)) == 0) {	/* On the sector boundary? */
			...
			if (cc) {						/* Write maximum contiguous sectors directly */
				if (csect + cc > fp->fs->csize)	/* Clip at cluster boundary */
					cc = fp->fs->csize - csect;
				if (diskWrite(fp->fs->drv, wbuff, sect, cc) != RES_OK)
					ABORT(fp->fs, FR_DISK_ERR);
#if _FS_MINIMIZE <= 2
#if _FS_TINY
			...
			if (fp->dsect != sect) {		/* Fill sector cache with file data */
				if (fp->fptr < fp->fsize &&
					diskRead(fp->fs->drv, fp->buf, sect, 1) != RES_OK)
						ABORT(fp->fs, FR_DISK_ERR);
			}
#endif
			fp->dsect = sect;
...
#endif
					if (diskRead(fp->fs->drv, fp->buf, dsc, 1) != RES_OK)	/* Load current sector */
						ABORT(fp->fs, FR_DISK_ERR);
#endif
					fp->dsect = dsc;
				}
...
#endif
			if (diskRead(fp->fs->drv, fp->buf, nsect, 1) != RES_OK)	/* Fill sector cache */
				ABORT(fp->fs, FR_DISK_ERR);
#endif
			fp->dsect = nsect;
		}
...
)
{
	...
	if (diskIoctl(pdrv, GET_SECTOR_SIZE, &SS(fs)) != RES_OK || SS(fs) > _MAX_SS || SS(fs) < _MIN_SS)
		return FR_DISK_ERR;
#endif
	if (_MULTI_PARTITION && part) {
		/* Get partition information from partition table in the MBR */
		if (diskRead(pdrv, fs->win, 0, 1) != RES_OK) return FR_DISK_ERR;
		...
		if (diskIoctl(pdrv, GET_SECTOR_COUNT, &n_vol) != RES_OK || n_vol < 128)
			return FR_DISK_ERR;
	...
	if (diskIoctl(pdrv, GET_BLOCK_SIZE, &n) != RES_OK || !n || n > 32768) n = 1;
	...
	if (_MULTI_PARTITION && part) {
		...
		tbl = &fs->win[MBR_Table + (part - 1) * SZ_PTE];
		tbl[4] = sys;
		if (diskWrite(pdrv, fs->win, 0, 1) != RES_OK)	/* Write it to teh MBR */
			return FR_DISK_ERR;
		md = 0xF8;
	} else {
		if (sfd) {	/* No partition table (SFD) */
			...
			if (diskWrite(pdrv, fs->win, 0, 1) != RES_OK)	/* Write it to the MBR */
				return FR_DISK_ERR;
			md = 0xF8;
		}
	}
	...
	if (diskWrite(pdrv, tbl, b_vol, 1) != RES_OK)	/* Write it to the VBR sector */
		return FR_DISK_ERR;
	...
	for (i = 0; i < N_FATS; i++) {		/* Initialize each FAT copy */
		...
		if (diskWrite(pdrv, tbl, wsect++, 1) != RES_OK)
			return FR_DISK_ERR;
	...
	return (diskIoctl(pdrv, CTRL_SYNC, 0) == RES_OK) ? FR_OK : FR_DISK_ERR;
}
...
)
{
	...
	if (stat & STA_NOINIT) return FR_NOT_READY;
	if (stat & STA_PROTECT) return FR_WRITE_PROTECTED;
	if (diskIoctl(pdrv, GET_SECTOR_COUNT, &sz_disk)) return FR_DISK_ERR;
	...
	/* Write it to the MBR */
	return (diskWrite(pdrv, buf, 0, 1) != RES_OK || diskIoctl(pdrv, CTRL_SYNC, 0) != RES_OK) ? FR_DISK_ERR : FR_OK;
}
