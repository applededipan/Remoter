...
{
#ifdef USE_SHIFT_ALG
	ch = ((ch<<4)&0xF0)|((ch>>4)&0x0F);
#endif
    if(chan == MAVLINK_COMM_0)
    {
        usart2UavSendChar(ch);       //! usart2UavPort
    }
    else if(chan == MAVLINK_COMM_1)
    {
    	usart1UsbSendChar(ch);       //! usart1UsbPort
    }
	else if(chan == MAVLINK_COMM_2)
	{
		usart4RspSendChar(ch);       //! usart4PaiPort
	}
	else if(chan == MAVLINK_COMM_3)                        
	{
		usart3BthSendChar(ch);       //! bluetoothPort
	}
	else
	{
		                             //! nothing to do
