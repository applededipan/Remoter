...
{
  /* Don't add YEAR to TM_YEAR_BASE, as that might overflow. Also, work even if YEAR is negative.  */
  return
...
ydhms_diff (long int year1, long int yday1, int hour1, int min1, int sec1, int year0, int yday0, int hour0, int min0, int sec0)
{
  verify (C99_integer_division, -1 / 2 == 0);
  verify (long_int_year_and_yday_are_wide_enough,
          INT_MAX <= LONG_MAX / 2 || TIME_T_MAX <= UINT_MAX);

  /* Compute intervening leap days correctly even if year is negative. Take care to avoid integer overflow here.  */
  ...
  /* Compute the desired time in gtime_t precision.  Overflow might occur here.  */
...
static gtime_t
guess_time_tm (long int year, long int yday, int hour, int min, int sec,  gtime_t *t, struct gtm *tp)
{
  if (tp)
    {
...
static struct gtm *
ranged_convert (struct gtm *(*convert) (gtime_t *, struct gtm *), gtime_t *t, struct gtm *tp)
{
...
gtime_t __mktime_internal (struct gtm *tp, struct gtm *(*convert) (gtime_t *, struct gtm *), gtime_t *offset)
{
