...
/****************************************************************
  * @brief  creat a bin file    added by apple 25/05/2016
...
uint8_t creatUpdateFile(uint8_t* data, uint32_t offset, uint8_t size)
{
	FATFS FatFs;
    FIL   updateFile;
	BYTE  buffer[256]; //! actually only use first 239 bytes
	UINT  count;	
    memcpy(&buffer, (BYTE*)data, size);
	memset(data, 0, size);	
	
	f_mount(&FatFs, "0:", 0);
	
	if(f_open(&updateFile, "0:SmartConsole/firmware/opentx.bin", FA_WRITE|FA_CREATE_ALWAYS)) 
	{ 
		return 1;	
	}
	if(f_lseek(&updateFile, offset))
	{
		return 2;	
	}		
	if(f_write(&updateFile, &buffer, size, &count))
	{
		return 3;	
	}		
...
uint32_t sdGetNoSectors()
{
  ...
  if(noSectors == 0) 
  {
    diskIoctl(0, GET_SECTOR_COUNT, &noSectors);
  }
  return noSectors;
}
