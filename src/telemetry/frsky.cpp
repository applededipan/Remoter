...
void telemetryWakeup()
{
  ...
  {
    usart2UavInit(requiredTelemetryProtocol);
...
/* #if defined(PCBTARANIS)
  ...
  gettime(&utm);
#endif
  while (telemetryrxFifo.pop(data)) 
  {
...
void telemetryInit(uint8_t protocol)
{
  ...
  {
    usart2UavInit(FRSKY_D_BAUDRATE);
  }
#if defined(PCBTARANIS)
  else if (protocol == PROTOCOL_PULSES_CROSSFIRE) 
  {
    usart2UavInit(CROSSFIRE_BAUDRATE);
  }
#endif
  else 
  {
    usart2UavInit(FRSKY_SPORT_BAUDRATE);
  }
